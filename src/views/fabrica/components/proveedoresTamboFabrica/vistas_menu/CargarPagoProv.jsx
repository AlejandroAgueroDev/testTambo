...
import AddEfectivoProv from "./metodosPago/AddEfectivo";
import AddCheque from "./metodosPago/AddCheque";
import AddOtrosProv from "./metodosPago/AddOtros";
import AddTransferenciaProv from "./metodosPago/AddTransferencia";
...
const CargarPagoProv = ({
  ...
  const [formData, setFormData] = useState({
    ...
    detalle: isLiquidacion ? "Liquidación de Retiros" : "",
    model: "TAMBO_PROVEEDOR",
    metodos: [],
  ...
  useEffect(() => {
    axios(url + "proveedor/pago/" + id_proveedor).then(({ data }) => {
  ...
  const handleSubmit = () => {
    ...
    setLoader(true);
    axios
      .post(url + "proveedor/pago", dataPago)
      .then((res) => {
        setLoader(false);
        Swal.fire({
          ...
          fetch();
          isLiquidar ? setCloseModal(true) : setCloseModal(false); 
        });
      })
      .catch((err) => {
        console.log(err);
